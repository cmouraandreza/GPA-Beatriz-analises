...
gen_epi_spp <- df1 %>%
  ...
  arrange(desc(n_epi))%>%
  rename(Genera = EP_Genera)  # se já for EP_family, use rename(family = EP_family)

gen_base_spp <- df1 %>%
  distinct(BS_Genera, Scientific_name_EP) %>%  # remove duplicatas de espécie dentro da família
  count(BS_Genera, name = "n_base") %>%    # conta quantas espécies únicas por família
  arrange(desc(n_base))%>%
